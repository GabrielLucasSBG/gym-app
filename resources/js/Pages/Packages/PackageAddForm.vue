<template>
    <form v-on:submit.prevent="handleFormSubmit">
        <div class="form-group">
            ...
            <div v-if="$page.props.errors.name" class="error">{{ $page.props.errors.name[0] }}</div>
        </div>

        <div class="form-group">
            <label for="amount">Amount</label>
            <input type="text" class="form-control" v-model="localPackage.amount"/>
            <div v-if="$page.props.errors.amount" class="error">{{ $page.props.errors.amount[0] }}</div>
        </div>

        <div class="form-group">
            <label for="amount">Number of days</label>
            <input type="number" class="form-control" v-model="localPackage.number_of_days"/>
            <div v-if="$page.props.errors.number_of_days" class="error">{{ $page.props.errors.number_of_days[0] }}</div>
        </div>
